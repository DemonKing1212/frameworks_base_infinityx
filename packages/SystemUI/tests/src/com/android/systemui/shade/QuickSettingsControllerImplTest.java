...
public class QuickSettingsControllerImplTest extends QuickSettingsControllerImplBaseTest {
    ...
    @Test
    public void testSplitShadeEnabledLegacy_forcedDisabledInLandscape() {
        // GIVEN legacy split shade is enabled (simulated by onTuningChanged)
        mQsController.getQsFragmentListener().onTuningChanged(
                "system:qs_split_shade", "1");
        ...
        // THEN getSplitShadeEnabledLegacy returns true
        assertThat(mQsController.getSplitShadeEnabledLegacy()).isTrue();

        // WHEN orientation is landscape
        config.orientation = android.content.res.Configuration.ORIENTATION_LANDSCAPE;

        // THEN getSplitShadeEnabledLegacy returns false
        assertThat(mQsController.getSplitShadeEnabledLegacy()).isFalse();
    }
